...
-export([mam_service_discovery/1,
         ...
         simple_text_serach_request/1,
         long_text_serach_request/1,
...
text_search_cases() ->
    [
     simple_text_serach_request,
     long_text_serach_request
    ].
...
init_per_testcase(C=prefs_set_cdata_request, Config) ->
    skip_if_riak(C, Config);
init_per_testcase(C=simple_text_serach_request, Config) ->
    skip_if_not_odbc(C, Config);
init_per_testcase(C=long_text_serach_request, Config) ->
    skip_if_not_odbc(C, Config);
...
simple_text_serach_request(Config) ->
    P = ?config(props, Config),
    F = fun(Alice, Bob) ->
...
long_text_serach_request(Config) ->
    P = ?config(props, Config),
    F = fun(Alice, Bob) ->
        ...
        Msgs =
            [
             ...
             <<"Culpa eu aliquip pork veniam dolor jowl id laborum in frankfurter ribs.">>,
             ...
             <<"Qui mollit short ribs, capicola bresaola culpa meatloaf kielbasa und pork.">>,
